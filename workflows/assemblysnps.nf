...
include { HARVESTTOOLS           } from '../modules/local/harvesttools/main'
...
workflow ASSEMBLYSNPS {
    ...
    //
    // Prepare assemblies for PARSNP
    //
    ch_assemblies = ch_samplesheet
        .map { meta, assembly -> 
            def new_meta = [id: "parsnp_analysis"]
            [new_meta, assembly]
        }
        .groupTuple()
    
    ch_reference = ch_samplesheet
        .first()
        .map { meta, assembly -> assembly }

    //
    // MODULE: Run PARSNP for core genome alignment
    //
    PARSNP (
        ch_assemblies,
        ch_reference
    ...
    //
    // MODULE: Convert PARSNP output to FASTA using HarvestTools
    //
    HARVESTTOOLS (
        PARSNP.out.ggr
    )
    ch_versions = ch_versions.mix(HARVESTTOOLS.out.versions.first())

    //
    // MODULE: Calculate SNP distances
    //
    SNPDISTS (
        HARVESTTOOLS.out.fasta
    )
    ch_versions = ch_versions.mix(SNPDISTS.out.versions.first())

    //
    // MODULE: Extract SNP sites
    //
    SNPSITES (
        HARVESTTOOLS.out.fasta
    )
    ...
    softwareVersionsToYAML(ch_versions)
        .collectFile(
            ...
            name: 'nf_core_'  +  'assemblysnps_software_'  + 'mqc_'  + 'versions.yml',
            sort: true,
            newLine: true
